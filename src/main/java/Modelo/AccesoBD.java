...
public class AccesoBD {
	...
	public List<Anuncio> filtrar (Distrito distrito, double pMinimo, double pMaximo, boolean parking, boolean terraza,
		...
		if (distrito != null) selectQueryBody += "AND H.DistritoNombre = ? ";
		if (parking) selectQueryBody += "AND H.Parking = ? ";
		if (terraza) selectQueryBody += "AND H.Terraza = ? ";
		if (wifi) selectQueryBody += "AND H.Wifi = ? ";
		if (fumadores) selectQueryBody += "AND H.Fumadores = ? ";
		if (mascotas) selectQueryBody += "AND H.Mascotas = ? ";
		if (estudiantes) selectQueryBody += "AND H.Solo_Estudiantes = ? ";
		if (gastos) selectQueryBody += "AND H.Gastos_Incluidos = ? ";
		...
		try {
			...
			if (distrito != null) {
				preparedStatement.setString(i, distrito.getNombre()); i++;
			}
			if (parking) {
				preparedStatement.setBoolean(i, parking); i++;
			}
			if (terraza) {
				preparedStatement.setBoolean(i, terraza); i++;
			}
			if (wifi) {
				preparedStatement.setBoolean(i, wifi); i++;
			}
			if (fumadores) {
				preparedStatement.setBoolean(i, fumadores); i++;
			}
			if (mascotas) {
				preparedStatement.setBoolean(i, mascotas); i++;
			}
			if (estudiantes) {
				preparedStatement.setBoolean(i, estudiantes); i++;
			}
			if (gastos) {
				preparedStatement.setBoolean(i, gastos); i++;
			}
			...
			while (rs.next()) {
				...
				Habitacion hab = new Habitacion(rs.getInt(1), rs.getString(2), rs.getInt(3),
						...
						rs.getBoolean(13), rs.getBoolean(14), true, propietario, dist);
	...
	public List<Anuncio> getFavoritos(Usuario u) throws Excepcion {
        String selectQueryBody = "SELECT * " + 
                "FROM Favoritos as F join Anuncio as A on (F.idAnuncio = A.idAnuncio) " + 
                "join Habitacion as H on (H.idHabitacion = F.idAnuncio) " + 
                "WHERE F.correoUsuario = ?";
        ...
        try {
            ...
            while(rs.next()) {
                ...
                Habitacion habitacion = new Habitacion(rs.getInt(2),rs.getString(8),rs.getInt(9),rs.getInt(10),rs.getString(11),rs.getString(12),rs.getInt(13),rs.getBoolean(14),rs.getBoolean(15),rs.getBoolean(16),rs.getBoolean(17),rs.getBoolean(18),rs.getBoolean(19),rs.getBoolean(20),true, propietario, distrito);
